...
def _identity_primary_key(connection) -> str:
...
def _ensure_postgres_id_default(connection, table_name: str) -> None:
    ...
    if not column_state:
        return
    column_default, is_identity = column_state
    if column_default or str(is_identity).upper() == "YES":
        return
    sequence_name = f"{table_name}_id_seq"
    connection.execute(text(f"CREATE SEQUENCE IF NOT EXISTS {sequence_name} OWNED BY {table_name}.id"))
    connection.execute(
        text(
            f"""
            SELECT setval(
...
def ensure_schema() -> None:
    ...
    with engine.begin() as connection:
        ...
        if "internal_operation_options" not in tables:
            ...
            connection.execute(text("CREATE INDEX IF NOT EXISTS ix_internal_operation_options_option_type ON internal_operation_options (option_type)"))
            connection.execute(text("CREATE INDEX IF NOT EXISTS ix_internal_operation_options_kind ON internal_operation_options (kind)"))
            current_tables = set(inspect(connection).get_table_names())
            _ensure_postgres_id_default(connection, "internal_operation_options")
        if "internal_operation_records" in tables:
            connection.execute(
                text(
                    """
